...
public class SpringTaskHandler {
    ...
    public void scheduile1() throws Exception {
        Thread.sleep(3000);
//        logger.debug("同步 每秒执行一次：{}", LocalDateTime.now());
    }
    ...
    public void scheduile2() throws Exception {
        Thread.sleep(3000);
//        logger.debug("异步 每秒执行一次：{}", LocalDateTime.now());
    }

    @Scheduled(fixedRate = 1000)
    public void scheduile3() throws Exception {
        Thread.sleep(3000);
//        logger.debug("固定周期 每秒执行一次：{}", LocalDateTime.now());
    }

    @Scheduled(fixedDelay = 5000)
    public void scheduile4() throws Exception {
        Thread.sleep(3000);
//        logger.debug("固定延迟 每秒执行一次：{}", LocalDateTime.now());
    }
}